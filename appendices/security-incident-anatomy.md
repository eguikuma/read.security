...
[03-certificate](../03-certificate.md) では DigiNotar と Symantec の認証局の信頼失墜、[06-application-security](../06-application-security.md) では Next.js ミドルウェアバイパス、[07-supply-chain](../07-supply-chain.md) では Log4Shell、xz-utils バックドア、event-stream 事件を学びました
...
1. [事件の一覧](#事件の一覧)
2. [共通パターンの抽出](#共通パターンの抽出)
3. [CVE と脆弱性開示](#cve-と脆弱性開示)
4. [事件から学ぶ設計原則](#事件から学ぶ設計原則)
5. [まとめ](#まとめ)
6. [用語集](#用語集)
7. [参考資料](#参考資料)
...
- [Common Vulnerability Scoring System v3.1](https://www.first.org/cvss/v3.1/specification-document)
  - CVSS の仕様書
