...
[01-cryptography](../01-cryptography.md) では、RSA が「大きな数の素因数分解の困難さ」に、楕円曲線暗号が「楕円曲線上の離散対数問題の困難さ」に安全性を依存していることを学びました

[02-tls](../02-tls.md) では、TLS 1.3 が ECDHE（楕円曲線 Diffie-Hellman）を鍵交換に使用していることを学びました

[03-certificate](../03-certificate.md) では、証明書のデジタル署名が公開鍵暗号に依存していることを学びました
...
1. [量子コンピュータと暗号の関係](#量子コンピュータと暗号の関係)
2. [非対称暗号への脅威](#非対称暗号への脅威)
3. [対称暗号とハッシュ関数への影響](#対称暗号とハッシュ関数への影響)
4. [Harvest Now, Decrypt Later](#harvest-now-decrypt-later)
5. [耐量子暗号（Post-Quantum Cryptography）](#耐量子暗号post-quantum-cryptography)
6. [TLS と PKI への影響](#tls-と-pki-への影響)
7. [まとめ](#まとめ)
8. [用語集](#用語集)
9. [参考資料](#参考資料)
...
[cryptography-history](cryptography-history.md) で学んだ AES の公開コンペと同様に、NIST は 2016 年に <strong>PQC 標準化プロジェクト</strong>を開始しました
